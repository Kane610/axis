...
async def test_aiohttp_digest_request_target_preencodes_query_params(
    ...
    no_params_url, no_params = axis_device.vapix._aiohttp_digest_auth.request_target(
        f"http://{HOST}/axis-cgi/io/port.cgi",
        None,
        True,
    )
...
async def test_aiohttp_digest_signature_validation_known_values(
    ...
    assert ha2 == hashlib.md5(b"GET:/axis-cgi/io/port.cgi?action=9%3A%5C").hexdigest()
