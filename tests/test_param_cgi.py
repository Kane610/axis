...
def test_update_brand(params):
    ...
    assert params[f"{BRAND}.Brand"].raw == "AXIS"
    assert params[f"{BRAND}.ProdFullName"].raw == "AXIS M1065-LW Network Camera"
    assert params[f"{BRAND}.ProdNbr"].raw == "M1065-LW"
    assert params[f"{BRAND}.ProdShortName"].raw == "AXIS M1065-LW"
    assert params[f"{BRAND}.ProdType"].raw == "Network Camera"
    assert params[f"{BRAND}.ProdVariant"].raw == ""
...
def test_update_ports(params):
    ...
    assert params[f"{INPUT}.NbrOfInputs"].raw == "1"
    assert params[f"{IOPORT}.I0.Configurable"].raw == "no"
    assert params[f"{IOPORT}.I0.Direction"].raw == "input"
    assert params[f"{IOPORT}.I0.Input.Name"].raw == "PIR sensor"
    assert params[f"{IOPORT}.I0.Input.Trig"].raw == "closed"
...
def test_update_properties(params):
    ...
    assert params[f"{PROPERTIES}.AlwaysMulticast.AlwaysMulticast"].raw == "yes"
    assert params[f"{PROPERTIES}.API.Browser.Language"].raw == "yes"
    assert params[f"{PROPERTIES}.API.Browser.RootPwdSetValue"].raw == "yes"
    assert params[f"{PROPERTIES}.API.Browser.UserGroup"].raw == "yes"
    assert params[f"{PROPERTIES}.API.ClientNotes.ClientNotes"].raw == "yes"
    assert params[f"{PROPERTIES}.API.HTTP.AdminPath"].raw == "/"
    assert params[f"{PROPERTIES}.API.HTTP.Version"].raw == "3"
    assert params[f"{PROPERTIES}.API.Metadata.Metadata"].raw == "yes"
    assert params[f"{PROPERTIES}.API.Metadata.Version"].raw == "1.0"
    assert params[f"{PROPERTIES}.API.OnScreenControls.OnScreenControls"].raw == "yes"
    assert params[f"{PROPERTIES}.API.PTZ.Presets.Version"].raw == "2.00"
    assert params[f"{PROPERTIES}.API.RTSP.RTSPAuth"].raw == "yes"
    assert params[f"{PROPERTIES}.API.RTSP.Version"].raw == "2.01"
    assert params[f"{PROPERTIES}.API.WebService.EntryService"].raw == "yes"
    assert params[f"{PROPERTIES}.API.WebService.WebService"].raw == "yes"
    assert params[f"{PROPERTIES}.API.WebService.ONVIF.ONVIF"].raw == "yes"
    assert params[f"{PROPERTIES}.API.WebService.ONVIF.Version"].raw == "1.02"
    assert params[f"{PROPERTIES}.API.WebSocket.RTSP.RTSP"].raw == "yes"
    assert params[f"{PROPERTIES}.ApiDiscovery.ApiDiscovery"].raw == "yes"
    assert params[f"{PROPERTIES}.Audio.Audio"].raw == "yes"
    assert params[f"{PROPERTIES}.Audio.DuplexMode"].raw == "half,post,get"
    assert params[f"{PROPERTIES}.Audio.Format"].raw == "lpcm,g711,g726,aac,opus"
    assert params[f"{PROPERTIES}.Audio.InputType"].raw == "internal"
    assert (
        params[f"{PROPERTIES}.Audio.Decoder.Format"].raw == "g711,g726,axis-mulaw-128"
    )
    assert params[f"{PROPERTIES}.Audio.Source.A0.Input"].raw == "yes"
    assert params[f"{PROPERTIES}.Audio.Source.A0.Output"].raw == "yes"
    assert params[f"{PROPERTIES}.EmbeddedDevelopment.CacheSize"].raw == "76546048"
    assert (
        params[f"{PROPERTIES}.EmbeddedDevelopment.DefaultCacheSize"].raw == "92274688"
    )
    assert params[f"{PROPERTIES}.EmbeddedDevelopment.EmbeddedDevelopment"].raw == "yes"
    assert params[f"{PROPERTIES}.EmbeddedDevelopment.Version"].raw == "2.16"
    assert (
        params[f"{PROPERTIES}.EmbeddedDevelopment.RuleEngine.MultiConfiguration"].raw
        == "yes"
    )
    assert params[f"{PROPERTIES}.Firmware.BuildDate"].raw == "Feb 15 2019 09:42"
    assert params[f"{PROPERTIES}.Firmware.BuildNumber"].raw == "26"
    assert params[f"{PROPERTIES}.Firmware.Version"].raw == "9.10.1"
    assert params[f"{PROPERTIES}.FirmwareManagement.Version"].raw == "1.0"
    assert params[f"{PROPERTIES}.GuardTour.GuardTour"].raw == "yes"
    assert params[f"{PROPERTIES}.GuardTour.MaxGuardTours"].raw == "100"
    assert params[f"{PROPERTIES}.GuardTour.MinGuardTourWaitTime"].raw == "10"
    assert params[f"{PROPERTIES}.GuardTour.RecordedTour"].raw == "no"
    assert params[f"{PROPERTIES}.HTTPS.HTTPS"].raw == "yes"
    assert params[f"{PROPERTIES}.Image.Format"].raw == "jpeg,mjpeg,h264"
    assert params[f"{PROPERTIES}.Image.NbrOfViews"].raw == "2"
    assert (
        params[f"{PROPERTIES}.Image.Resolution"].raw
        == "1920x1080,1280x960,1280x720,1024x768,1024x576,800x600,640x480,640x360,352x240,320x240"
    )
    assert params[f"{PROPERTIES}.Image.Rotation"].raw == "0,180"
    assert params[f"{PROPERTIES}.Image.ShowSuboptimalResolutions"].raw == "false"
    assert params[f"{PROPERTIES}.Image.H264.Profiles"].raw == "Baseline,Main,High"
    assert params[f"{PROPERTIES}.ImageSource.DayNight"].raw == "yes"
    assert params[f"{PROPERTIES}.IO.ManualTriggerNbr"].raw == "6"
    assert params[f"{PROPERTIES}.LEDControl.LEDControl"].raw == "yes"
    assert params[f"{PROPERTIES}.LightControl.LightControl2"].raw == "yes"
    assert params[f"{PROPERTIES}.LightControl.LightControlAvailable"].raw == "yes"
    assert params[f"{PROPERTIES}.LocalStorage.AutoRepair"].raw == "yes"
    assert params[f"{PROPERTIES}.LocalStorage.ContinuousRecording"].raw == "yes"
    assert params[f"{PROPERTIES}.LocalStorage.DiskEncryption"].raw == "yes"
    assert params[f"{PROPERTIES}.LocalStorage.DiskHealth"].raw == "yes"
    assert params[f"{PROPERTIES}.LocalStorage.ExportRecording"].raw == "yes"
    assert params[f"{PROPERTIES}.LocalStorage.FailOverRecording"].raw == "yes"
    assert params[f"{PROPERTIES}.LocalStorage.LocalStorage"].raw == "yes"
    assert (
        params[f"{PROPERTIES}.LocalStorage.NbrOfContinuousRecordingProfiles"].raw == "1"
    )
    assert params[f"{PROPERTIES}.LocalStorage.RequiredFileSystem"].raw == "yes"
    assert params[f"{PROPERTIES}.LocalStorage.SDCard"].raw == "yes"
    assert params[f"{PROPERTIES}.LocalStorage.StorageLimit"].raw == "yes"
    assert params[f"{PROPERTIES}.LocalStorage.Version"].raw == "1.00"
    assert params[f"{PROPERTIES}.Motion.MaxNbrOfWindows"].raw == "10"
    assert params[f"{PROPERTIES}.Motion.Motion"].raw == "yes"
    assert params[f"{PROPERTIES}.Network.WLAN.WLANScan2"].raw == "yes"
    assert params[f"{PROPERTIES}.NetworkShare.CIFS"].raw == "yes"
    assert params[f"{PROPERTIES}.NetworkShare.IPV6"].raw == "yes"
    assert params[f"{PROPERTIES}.NetworkShare.NameLookup"].raw == "yes"
    assert params[f"{PROPERTIES}.NetworkShare.NetworkShare"].raw == "yes"
    assert params[f"{PROPERTIES}.PackageManager.FormatListing"].raw == "yes"
    assert params[f"{PROPERTIES}.PackageManager.LicenseKeyManagement"].raw == "yes"
    assert params[f"{PROPERTIES}.PackageManager.PackageManager"].raw == "yes"
    assert params[f"{PROPERTIES}.PrivacyMask.MaxNbrOfPrivacyMasks"].raw == "10"
    assert params[f"{PROPERTIES}.PrivacyMask.Polygon"].raw == "no"
    assert params[f"{PROPERTIES}.PrivacyMask.PrivacyMask"].raw == "no"
    assert (
        params[f"{PROPERTIES}.PrivacyMask.Query"].raw
        == "list,position,listpxjson,positionpxjson"
    )
    assert params[f"{PROPERTIES}.PTZ.DigitalPTZ"].raw == "yes"
    assert params[f"{PROPERTIES}.PTZ.DriverManagement"].raw == "no"
    assert params[f"{PROPERTIES}.PTZ.DriverModeList"].raw == "none"
    assert params[f"{PROPERTIES}.PTZ.PTZ"].raw == "yes"
    assert params[f"{PROPERTIES}.PTZ.PTZOnQuadView"].raw == "no"
    assert params[f"{PROPERTIES}.PTZ.SelectableDriverMode"].raw == "no"
    assert params[f"{PROPERTIES}.RemoteService.RemoteService"].raw == "no"
    assert params[f"{PROPERTIES}.RTC.RTC"].raw == "yes"
    assert params[f"{PROPERTIES}.Sensor.PIR"].raw == "yes"
    assert params[f"{PROPERTIES}.Serial.Serial"].raw == "no"
    assert params[f"{PROPERTIES}.System.Architecture"].raw == "armv7hf"
    assert params[f"{PROPERTIES}.System.HardwareID"].raw == "70E"
    assert params[f"{PROPERTIES}.System.Language"].raw == "English"
    assert params[f"{PROPERTIES}.System.LanguageType"].raw == "default"
    assert params[f"{PROPERTIES}.System.SerialNumber"].raw == "ACCC12345678"
    assert (
        params[f"{PROPERTIES}.System.Soc"].raw
        == "Ambarella S2L (Flattened Device Tree)"
    )
    assert params[f"{PROPERTIES}.Tampering.Tampering"].raw == "yes"
    assert params[f"{PROPERTIES}.TemperatureSensor.Fan"].raw == "no"
    assert params[f"{PROPERTIES}.TemperatureSensor.Heater"].raw == "no"
    assert params[f"{PROPERTIES}.TemperatureSensor.TemperatureControl"].raw == "yes"
