...
def test_update_properties():
    ...
    assert (
        params[PROPERTIES + ".Audio.Decoder.Format"].raw == "g711,g726,axis-mulaw-128"
    )
    ...
    assert (
        params[PROPERTIES + ".EmbeddedDevelopment.DefaultCacheSize"].raw == "92274688"
    )
    ...
    assert (
        params[PROPERTIES + ".EmbeddedDevelopment.RuleEngine.MultiConfiguration"].raw
        == "yes"
    )
    ...
    assert (
        params[PROPERTIES + ".Image.Resolution"].raw
        == "1920x1080,1280x960,1280x720,1024x768,1024x576,800x600,640x480,640x360,352x240,320x240"
    )
    ...
    assert (
        params[PROPERTIES + ".LocalStorage.NbrOfContinuousRecordingProfiles"].raw == "1"
    )
    ...
    assert (
        params[PROPERTIES + ".PrivacyMask.Query"].raw
    ...
    assert (
        params[PROPERTIES + ".System.Soc"].raw
