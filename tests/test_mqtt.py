...
@pytest.fixture
def mqtt_client(device) -> MqttClient:
    """Returns the mqtt_client mock object."""
    return MqttClient(device.vapix.request)


@respx.mock
async def test_client_config_simple(mqtt_client):
    """Test simple MQTT client configuration."""
    route = respx.post("http://host:80/axis-cgi/mqtt/client.cgi")
...
async def test_get_event_publication_config(mqtt_client):
    ...
    route = respx.post("http://host:80/axis-cgi/mqtt/event.cgi").respond(
        json={
            "apiVersion": "1.0",
            "context": "Axis lib",
...
async def test_configure_event_publication_all_topics(mqtt_client):
    ...
    route = respx.post("http://host:80/axis-cgi/mqtt/event.cgi")
